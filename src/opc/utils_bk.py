...
def segment_lines(lines, seg_length):
    ...
    Args:
        lines (torch.Tensor): Tensor of shape [N, 2, D] representing the lines.
...
def segments_merge2polygon(segments):
                ...
                if all_fake_eps:
                    ...
                    if any(ep[0] in fp or ep[1] in fp for fp in all_fake_eps):
                        ...
                        if all(e in sum(all_fake_eps, ()) for e in ep):
                            polygon_by_sp.append(start_pair)
                            all_sps.remove(start_pair)
                            all_polygons_by_start_pairs.append(polygon_by_sp)
                            polygon_by_sp = []
                            all_fake_eps = []
                        else:
    ...
    recursion_polygon(all_start_pairs, all_end_pairs)
    # print(f"number of polygons: {len(all_polygons_by_start_pairs)}")
    # print(all_polygons_by_start_pairs)
    all_polygons_by_start_id = [
        [element for tuple_pair in sublist for element in tuple_pair]
        for sublist in all_polygons_by_start_pairs
    ]
    # print(all_polygons_by_start_id)

    from itertools import chain

    all_polygons_all_ids = [
        list(chain.from_iterable(get_sub_by_start(seg_id) for seg_id in polygon))
        for polygon in all_polygons_by_start_id
    ]
    # print(all_polygons_all_ids)
    ...
    # return all_traverse_list, all_polygons_all_ids
    def group_by_poly_id(data):
        sorted_data = sorted(data, key=lambda x: x["polygon_id"])
        grouped_data = {}
        for poly_id, group in groupby(sorted_data, key=lambda x: x["polygon_id"]):
            grouped_data[poly_id] = list(group)
        return grouped_data

    # all_polygon_ids = []
    for seg in segments:
        poly_id = get_poly_id(all_polygons_all_ids, seg["id"])
        seg["polygon_id"] = poly_id
        # all_polygon_ids.append(poly_id)
    # all_polygon_ids = list(set(all_polygon_ids))

    # now every segment has a polygon_id
    segs = group_by_poly_id(segments)

    print(segs)

    def extract_vertices(data):
        vertices_result = {}
        for polygon_id, polygon_data in data.items():
            vertices = []
            ...
            if vertices:
                vertices_result[polygon_id] = torch.unique(torch.stack(vertices), dim=0)
        return vertices_result

    vertices_dict = extract_vertices(segs)
    print(vertices_dict)

    # def extract_segments_and_vertices(data):
    #     segments_result = {}
    #     vertices_result = {}
    #     for polygon_id, polygon_data in data.items():
    #         segments = []
    ...
    #                 vertices.append(end_point)
    #         if segments:
    #             segments_result[polygon_id] = torch.stack(segments)
    #         if vertices:
    #             vertices_result[polygon_id] = torch.unique(torch.stack(vertices), dim=0)
    #     return segments_result, vertices_result
    # _, vertices_dict = extract_segments_and_vertices(segs)
    # print(vertices_dict)

    # def sort_polygon_vertices_tensor(vertices):
    ...
    #     sorted_vertices = torch.cat((sorted_vertices, sorted_vertices[0].unsqueeze(0)), dim=0)
    #     return sorted_vertices

    img = torch.zeros((1, 1, 1280, 1280), dtype=torch.uint8)
    for _, value in vertices_dict.items():
        # print(f"before sort: {value}")
        # vertices = sort_polygon_vertices_tensor(value)
        # print(f"after sort: {vertices}")
        vertices = value.cpu().unsqueeze(0)

        color = torch.tensor((255,), dtype=torch.uint8)
        img = draw_convex_polygon(img, vertices, color)
    # img = torch.rot90(img, 1, [2, 3]).flip(2)
    plt.imshow(img.squeeze(0).permute(1, 2, 0).numpy())
    plt.show()
    return all_polygons_all_ids
...
def visual_seg2poly(segs, metadata):
    polygon1 = [seg["segment"] for seg in segs if seg["polygon_id"] == 1]
    # print(polygon1)
    # polygon1 = []
    # for seg in segs:
    #     if seg["id"] == 1:
    #         polygon1.append(seg["segment"])
    # print(polygon1)
...
def get_poly_id(all_polygons_all_ids, seg_id):
    for idx, polygon in enumerate(all_polygons_all_ids):
        if seg_id in polygon:
            return idx
